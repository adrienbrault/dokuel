...
- **Squash before merge**: `fixup!` commits are staging markers, not history. Run `git rebase -i --autosquash <base>` on the branch before merging so they fold into their parents — `fixup!` subjects must never land on `main`.
...
`bun run screenshots` writes 17 combined sheets under `e2e/screenshots/combined/`
...
- `feature--<group>.png` (×9) — related scenes across all 4 devices, devices as rows. Groups: `onboarding`, `landing-demo`, `solo`, `challenge`, `numpad`, `multiplayer`, `multiplayer-post-game`, `dark-mode-a`, `dark-mode-b`. Use when you want to compare a feature across viewports.
