...
test("solo resumes and starts a new puzzle without a network connection", async ({ page, context }) => {
  await page.goto("/solo/easy/offline-resume");
  await page.getByRole("button", { name: /, empty/ }).first().click();
  await page.getByRole("group", { name: "Number pad" }).getByRole("button", { name: "5", exact: true }).click();
