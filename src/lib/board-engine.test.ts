...
describe("PLACE_NUMBER_AT action", () => {
  const puzzle =
    "53..7....6..195....98....6.8...6...34..8.3..17...2...6.6....28....419..5....8..79";

  it("places a value at an explicit cell with no cell selected", () => {
    let state = initState({ puzzle });
    state = reducer(state, {
  ...
  it("does not overwrite a cell that already holds a value", () => {
    let state = initState({ puzzle });
    state = reducer(state, {
      type: "PLACE_NUMBER_AT",
      row: 0,
  ...
  it("records history so the placement can be undone", () => {
    let state = initState({ puzzle });
    state = reducer(state, {
      type: "PLACE_NUMBER_AT",
      row: 0,
  ...
  it("auto-eliminates the placed value from peer notes when asked", () => {
    let state = initState({ puzzle });
