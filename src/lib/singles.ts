/**
 * The single-cell scans that make up the first two rungs of the hint
 * ladder, plus the candidate arithmetic they share. They live apart
 * from hint-engine so that module can stay what it is: the ladder,
 * read top to bottom.
 */

import type { Board, PlacementHint, Position } from "./types.ts";
...
export function eliminatingCells(
  board: Board,
  row: number,
  col: number,
): Position[] {
...
export function findNakedSingle(board: Board): PlacementHint | null {
...
export function findHiddenSingle(board: Board): PlacementHint | null {
  for (let row = 0; row < 9; row++) {
