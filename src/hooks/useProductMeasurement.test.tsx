...
it("counts an attempt start once across rerenders", () => {
  const screen: Screen = { name: "solo", difficulty: "easy", gameKey: "attempt", assistLevel: "standard" };
  const { rerender } = renderHook(() => useProductMeasurement(screen));
  rerender();
  expect(trackProductEvent).toHaveBeenCalledExactlyOnceWith("game_start", "solo");
});
