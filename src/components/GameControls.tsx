...
export function GameControls({
  ...
  return (
    <div className="flex items-center justify-center gap-2">
      ...
      >
        <Undo2 size={19} strokeWidth={2.2} aria-hidden="true" />
      </ControlButton>
      <ControlButton label="Erase" onClick={onErase}>
        <Eraser size={19} strokeWidth={2.2} aria-hidden="true" />
      </ControlButton>
      {onHint && (
        <ControlButton label="Hint" onClick={onHint}>
          <Lightbulb size={19} strokeWidth={2.2} aria-hidden="true" />
        </ControlButton>
      )}
    </div>
  );
  ...
  return (
    <button
      ...
      className={`flex flex-col items-center justify-center gap-1 w-[4.75rem] py-2 rounded-xl border select-none touch-manipulation transition-colors ${
        disabled
          ? "border-transparent bg-bg-inset text-text-disabled cursor-default"
