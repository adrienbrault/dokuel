...
Hooks in `.Codex/settings.json` automate quality checks — **do not duplicate their work manually**:
...
- Updating AGENTS.md workflow instructions → `docs: ...` or `chore: ...`
...
**Never batch multiple tests before implementing.** See `.Codex/skills/tdd/SKILL.md` for the full TDD workflow.
...
Follow the TDD skill in `.Codex/skills/tdd/SKILL.md`. Key rules:
...
- **Mock only at system boundaries**: See `.Codex/skills/tdd/mocking.md`.
