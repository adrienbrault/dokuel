...
- "Beat my time" challenges — a solo win offers a link to the same seeded
  board carrying the winner's time and name
  (`/solo/<difficulty>/<key>?t=<seconds>&by=<name>`), shared via the native
  share sheet or copied to the clipboard. The receiver sees a banner above
  the board naming the time to beat, and the result dialog settles the race
  ("You beat Swift Panda's 04:12!" / "Swift Panda was faster: 04:12") before
  offering the same link with the new time. Malformed params are dropped, not
  fatal
...
- Boards are frozen in a static table (`src/lib/dailies.json`, 2026-05-01
  through 2027-12-31) generated by `bun run dailies:generate`. The table is
  the source of truth for those dates, so a change to the generator, the clue
  bands, or the grader can no longer fork the daily; dates outside it fall
  back to the seeded generator (same date, same board on any device)
- The table loads as its own chunk, so solo and multiplayer players never
  download it; the daily shows a caption for the tick it takes
- Medium difficulty
- Streak tracking — current streak and longest streak shown on landing page.
  Only today's date moves the streak
- Archive — `/daily/archive` lists past dailies newest first, grouped by
  month, marking which were solved and in what time, paged 45 dates at a
  time. Reached from a secondary link on the landing page and from the daily
  result dialog
- `/daily/<YYYY-MM-DD>` plays any date from 2026-05-01 up to today; anything
  else canonicalizes to `/daily`. Completing an archived daily records its
  result and its checkmark but leaves the streak untouched
- Per-date results (time + completion moment) stored in localStorage; an
  archived daily shares its date and its own `/daily/<date>` link
