...
async function writeSheet(
  ...
  const sheetW = colWidths.reduce((a, b) => a + b, 0) + GUTTER * (colCount + 1);
  const sheetH =
    HEADER_HEIGHT +
...
async function buildDevicePartSheet(
  ...
  const scenes = part.scenes.filter((scene) => idx.has(`${scene}--${device}`));
...
async function buildFeatureSheet(
  ...
  for (const device of devices) {
    ...
    for (const scene of group.scenes) {
      const shot = idx.get(`${scene}--${device}`);
      if (!shot) continue;
      row.push(await buildCell(shot, `${scene} · ${deviceLabel(device)}`));
    }
